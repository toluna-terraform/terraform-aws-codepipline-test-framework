...
func TestRunner(t *testing.T) {
	...
	expectedAppName := fmt.Sprintf("terratest-test-framework-%s", random.UniqueId())
	expectedEnvType := fmt.Sprintf("terratest-env-type-%s", random.UniqueId())
	expectedAppEnvs := fmt.Sprintf("terratest-app-envs-%s", random.UniqueId())
