Copy of newman-reporter-junifull with minor changes for personal proposes

# newman-reporter-junitfullreport
...
|-------------|-------------------|
| `--reporter-junitfullreport-export <path>` | Specify a path where the output XML file will be written to disk. If not specified, the file will be written to `newman/` in the current working directory. |
...
| **newman-reporter-junitfullreport** | **newman** | **node** |
|:-----------------------------:|:----------:|:--------:|
|            v1.0.0             | >= v4.0.0  | >= v6.x  |
|            v1.1.0             | >= v4.0.0  | >= v6.x  |
